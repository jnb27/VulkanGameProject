...
void gf3d_sprite_draw(Sprite *sprite, Vector2D position, Vector2D scale, Uint32 frame, Uint32 buffer_frame, VkCommandBuffer commandBuffer)
{
	...
	{
		slog("failed to get a free Set for sprite rendering");
		return;
	}
	gfc_matrix_scale(
		modelMat,
...
{
	int i;
	for (i = 0; i < gf3d_sprite.sprite_list; i++)
	{
		if (!gf3d_sprite.sprite_list[i]._inuse)continue;
		gf3d_sprite_draw(&gf3d_sprite.sprite_list[i], gf3d_sprite.sprite_list[i].position, gf3d_sprite.sprite_list[i].scale, 0, bufferFrame, commandBuffer);
	}
}
